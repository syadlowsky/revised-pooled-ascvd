...
examples[,"model.set.3.white.risk"] <- predict(logistic.2.eq, examples, recalculate=T)
examples$race <- 1
examples$grp <- 2 * examples$sex + 1
examples[,"model.set.3.black.risk"] <- predict(logistic.2.eq, examples, recalculate=T)
...
displayable[,"model.set.3.ratio"] <- specify.decimal(
    examples[,"model.set.3.black.risk"] / examples[,"model.set.3.white.risk"], nsmall=1)

write.csv(displayable, row.names=F)
